...
const Nav = () => {
 ...
 bg-[#f8fafc54] w-full shadow-md flex flex-row-reverse sm:flex-row justify-between items-center"
    >
      ...
      <div className="hidden sm:flex flex-row-reverse sm:ml-5 gap-8 items-center text-white font-medium">
        {navLinks.map((link, index) => (
          <a
            ...
            className="hover:text-[#164e63] transition"
          >
            {link.label}
          </a>
        ))}
          ...
          <div className="flex flex-col p-4 space-y-4">
            {navLinks.map((link, index) => (
              <a
                ...
                href={link.href}
                onClick={(e) => smoothScroll(e, link.href)}
                className="text-white hover:text-[#164e63] 
                transition-all duration-300 transform hover:scale-105 hover:font-bold 
                text-xl font-semibold py-3 px-4 text-center 
